...
- The **PROVIDER_KEY**, which can be found in the Account tab of your admin portal (only visible to the users with "admin" role).
...
The `3scale-cms` command should be invoked from the root directory of the folder where your local copy of the CMS will be stored and worked on.
...
When a local file of type `.html` or `.html.liquid` is created that does not have the `_` (underscore) prefix to indicate it is a partial, then the tool cannot tell if it is intended as a new page or new layout.
